...
public class Water : MonoBehaviour
{
     ...
     */
    public void floatOnWater(GameObject player)
    {
        if (!player.CompareTag("Player")) return;

        float waterSurface = transform.position.y + transform.localScale.y / 2; //물 표면의 y좌표
        float submergedRate = (waterSurface - (player.transform.position.y - _sizeOfBalloon / 2))/_sizeOfBalloon; //물에 잠긴 비율
        if (submergedRate > 1f) submergedRate = 1f;
